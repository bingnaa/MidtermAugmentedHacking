...
//        }.store(in: &subscriptions)
    ...
    func processUISignal(_ signal: ViewModel.UISignal) {
        ...
        case .reset:
            resetScene()
        case .spawnParasite:
            instantiateParasitesPool(count: 30)
        }
    }
    ...
    func renderTime(){
        if let gyroX = manager.gyroData?.rotationRate.x, gyroX > 2 {
            //print("gyroX:", gyroX)
            //if let userAccelerationX = manager.deviceMotion?.userAcceleration.x, userAccelerationX >  0.3 {
